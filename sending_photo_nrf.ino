...
void sendFromSDCard(int jpg_number) {
    ...
    while (file.available() || (ack_checksum != 2)) {
        Serial.print("ack id: ");
        Serial.print(ack_id);
        Serial.println();
        ...
        if(next_packet && file.available()) { // will be true if we need to go to next packet or else leave it the same and resend it
          bytesRead = file.read(buffer, (PACKET_SIZE - 2)); // 2 bytes off, one for checksum one for packet id
          buffer[26] = packet_id;
          buffer[27] = 0;
          next_packet = false;
          Serial.println(F("Inside read into packet"));
        }
        ...
        if ( nrf24.waitAvailableTimeout(2000) ) {
            ...
            if (  nrf24.recv(buf, &len) ) {
                ...
                Serial.print(ack_checksum);
                Serial.println();

                if (((ack_id == ((packet_id + 1) % 256)) && (ack_checksum == 0))) {  // CHANGED CODE
  ...
  for (k = 3; k < 4; k++) {
      ...
      strcat(str, ".jpg");
        ...
        if (outFile) {
            ...
            Serial.println(F("File open failed. Inside save to sd card. Trying again..."));
        }
